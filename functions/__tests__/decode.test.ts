...
it("parses a partial JSON response", () => {
  const r = lib.decodeCo2Response(
    JSON.parse(
      '{"_disclaimer":"This data is the exclusive property of Tomorrow and/or related parties. If you\'re in doubt about your rights to use this data, please contact hello@tmrow.com","status":"ok","countryCode":"US","data":{"fossilFuelPercentage":null},"units":{"carbonIntensity":"gCO2eq/kWh"}}'
    )
  );
...
it("parses a full JSON response", () => {
  ...
  expect(r).toEqual(
    Either.Right.create({
      value0: {
        carbonData: {
          value0: {
            carbonIntensity: 273.4587121483331,
            fossilFuelPercentage: 48.78024921423173
          }
        },
  ...
  expect(r.value0).toHaveProperty(
    "value0.carbonData.value0.carbonIntensity",
    273.4587121483331
  );
});
