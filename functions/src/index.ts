 ...
 */
const ssml = (template: TemplateStringsArray, ...inputs: string[]): string =>
  template
    .reduce((out, str, i) =>
      i
        ? out +
...
const Responses = {
  errorResolvingLocation: (err: string) =>
    ssml`<speak>
      There was an error while determining your location: ${err}.
  ...
  permissionReason: () => "To find out your local electricity source",
...
const isStorageExpired = (storage: IUserStorage): boolean =>
...
app.intent(Actions.REQUEST_LOC_PERMISSION, conv => {
  ...
  const data = conv.data as IStoredData;
  data.requestedPermission = requestedPermission;

  const storage = conv.user.storage as IUserStorage;
